...
void apply_volume_fadein(float volume, void *buf, int sample_size, int bytes)
{
	int16_t *input16 = (int16_t *)buf;
    int32_t *input32 = (int32_t *)buf;
    unsigned int i = 0;
    unsigned int frameCount = bytes / sample_size;
    if (sample_size == 2) {
        for (i = 0; i < frameCount; i++) {
            int32_t samp = (int32_t)(input16[i]);
            input16[i] = clamp16((int32_t)(samp * volume * i / frameCount));
        }
    } else if (sample_size == 4) {
        for (i = 0; i < frameCount; i++) {
            int64_t samp = (int64_t)(input32[i]);
            input32[i] = clamp32((int64_t)(samp * volume * i / frameCount));
        }
    } else {
        ALOGE("%s, unsupported audio format: %d!\n", __FUNCTION__, sample_size);
    }
...
void apply_volume_fadeout(float volume, void *buf, int sample_size, int bytes)
{
	int16_t *input16 = (int16_t *)buf;
    int32_t *input32 = (int32_t *)buf;
    unsigned int i = 0;
    ...
    if (sample_size == 2) {
        for (i = 0; i < frameCount; i++) {
            int32_t samp = (int32_t)(input16[i]);
            input16[i] = clamp16((int32_t)(samp * volume * (frameCount - i )/ frameCount));
        }
    } else if (sample_size == 4) {
        for (i = 0; i < frameCount; i++) {
            int64_t samp = (int64_t)(input32[i]);
            input32[i] = clamp32((int64_t)(samp * volume * (frameCount - i ) / frameCount));
        }
    } else {
        ALOGE("%s, unsupported audio format: %d!\n", __FUNCTION__, sample_size);
    }
